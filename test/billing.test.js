...
before(async () => {
  dataDir = mkdtempSync(path.join(tmpdir(), "cartoon-billing-"));
...
test("an unsigned webhook is refused before any user is touched", async () => {
  ...
  const payload = JSON.stringify({
    ...
    data: { object: { client_reference_id: user.id, metadata: { type: "pack", credits: "250" } } },
  });
...
test("a signed credit-pack purchase grants credits once", async () => {
  ...
  const payload = JSON.stringify({
    id: "evt_pack_1",
    type: "checkout.session.completed",
    data: { object: { client_reference_id: user.id, customer: "cus_1", metadata: { type: "pack", credits: "250" } } },
  });

  const res = await billing.handleWebhook(payload, sign(payload));
  assert.equal(res.ok, true);
  assert.equal((await store.getUserById(user.id)).bonusCredits, 250);
});
...
test("replaying the same credit-pack event does not grant credits twice", async () => {
  ...
  const payload = JSON.stringify({
    id: "evt_pack_replay",
    type: "checkout.session.completed",
    data: { object: { client_reference_id: user.id, customer: "cus_2", metadata: { type: "pack", credits: "600" } } },
  });
  const header = sign(payload);

  await billing.handleWebhook(payload, header);
  assert.equal((await store.getUserById(user.id)).bonusCredits, 600);
  ...
  const other = JSON.stringify({
    id: "evt_pack_second",
    type: "checkout.session.completed",
    data: { object: { client_reference_id: user.id, customer: "cus_2", metadata: { type: "pack", credits: "60" } } },
  });
  await billing.handleWebhook(other, sign(other));
  assert.equal((await store.getUserById(user.id)).bonusCredits, 660);
});
...
test("an event for an unknown user is accepted but changes nothing", async () => {
  const payload = JSON.stringify({
    id: "evt_ghost",
    type: "checkout.session.completed",
    data: { object: { client_reference_id: "no-such-user", metadata: { type: "pack", credits: "250" } } },
  });
  const res = await billing.handleWebhook(payload, sign(payload));
  assert.equal(res.ok, true);
});
...
test("a pack with no dashboard Price is charged inline, as a one-off", () => {
  const item = billing.buildLineItem(findPack("pack250"));
  ...
  assert.match(item["line_items[0][price_data][product_data][name]"], /Cartoon Studio/);
...
test("a plan is charged inline with a recurring interval", () => {
  ...
  assert.equal(item["line_items[0][price_data][unit_amount]"], "1200");
...
test("stripeEnabled and creditPacksEnabled are off without a secret key", () => {
...
test("checkout rejects anything not in the catalog, before reaching Stripe", async () => {
