// Persistence for Cartoon Studio with two interchangeable backends, same async
// API:
//   • Postgres  — used when DATABASE_URL is set (durable, multi-instance).
//   • JSON file — fallback for local dev / demo (no DB required).
//
// User records are stored as JSONB so the user shape can evolve freely; `id`,
// `email`, and `stripe_customer` are pulled out as columns for lookup.
...
function makeFile() {
  ...
  const FILE = path.join(DATA_DIR, "cartoon-studio.json");
  let db = { users: {} };
  let timer = null;
