// Anthropic integration for Cartoon Studio.
...
// Creative work wants warmth; structured extraction wants precision.
const CREATIVE_TEMP = clampTemp(process.env.AI_TEMPERATURE, 0.85);
...
// Vision: read an uploaded photo and describe it for cartoon conversion.
export async function visionExtract({ base64, mediaType, system, instruction, demo }) {
  if (!aiEnabled) return typeof demo === "function" ? demo() : demo;
  const content = [
  ...
  return parseLooseJSON(raw, typeof demo === "function" ? demo() : demo);
}
