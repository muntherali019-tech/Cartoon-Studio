// Accounts, password hashing, signed tokens, monthly credit tracking, the
...
// Which plans may use premium tools (Character Kit, Comic Studio, etc.).
...
export function publicUser(user) {
  ...
  return {
    ...
    bonusCredits: bonus,
    // Total credits still spendable this month (monthly + purchased packs).
    creditsLeft: allow === Infinity ? "unlimited" : monthlyLeft + bonus,
    referralCode: user.referralCode || null,
    referrals: user.referrals || 0,
    characterKit: user.characterKit || null,
  };
}
...
export async function setCharacterKit(user, kit = {}) {
  if (!user) throw new Error("Sign in first");
  if (!PREMIUM_PLANS.has(user.plan)) throw new Error("Character Kit is a Creator/Studio feature");
  ...
  user.characterKit = {
    ...
    style: clean(kit.style, 40),          // e.g. "Saturday-morning cartoon", "manga"
    primary: color(kit.primary, "#FF5C7A"),
    secondary: color(kit.secondary, "#5B8CFF"),
    outline: color(kit.outline, "#141821"),
    traits: clean(kit.traits, 240),       // recurring look/personality guidance for the AI
  };
  await saveUser(user);
  return user.characterKit;
}
